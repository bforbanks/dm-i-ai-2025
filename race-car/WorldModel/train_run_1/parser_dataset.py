...
from parser_features import precompute_parser_features
...
class LaneShiftDatasetWithParser(LaneShiftDataset):
    ...
    def __init__(
        ...
        self.parser_feats = precompute_parser_features(self.sensors, ego_y, lengths)
        print(f"  parser_feats shape={self.parser_feats.shape}", flush=True)
...
class LaneShiftGameDatasetWithParser(LaneShiftGameDataset):
    ...
    def __init__(
        ...
        lengths = d["game_lengths"].astype(np.int64)
        ego_y = d["ego_xy"][:, 1].astype(np.float32)
        print("Precomputing SensorParser features (full games) …", flush=True)
        self.parser_feats = precompute_parser_features(self.sensors, ego_y, lengths)
        print(f"  parser_feats shape={self.parser_feats.shape}", flush=True)
