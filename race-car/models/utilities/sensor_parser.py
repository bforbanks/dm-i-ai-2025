...
def parse_sensors(state: dict, previous_info, y_pos):
    ...
    for degree, name in sensor_options:
        ...
        if detected_value:
            ...
            if 50 <= y <= 1150:
                for i, (lo, hi) in enumerate(NPC_car_y_coordinate_ranges):
                    if lo-3 <= y <= hi+3: #buffer as there are some inaccuracies
                        if y-lo < 3 or hi-y < 3: # Inexact
                            ...
                            if previous_info[i]: # update velocity if possible.
                                for prev_information in previous_info[i]:
                                    if prev_information["type"] == "exact":
                                        information[i][-1]["velocity"] = information[i][-1]["x"]-prev_information["x"]
    previous_info = information
    parsed_info = []
