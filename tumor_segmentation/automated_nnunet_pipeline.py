...
def create_fresh_resenc_plans(dataset_id: int, patch_size: Tuple[int, int], batch_size: int, config_name: str, smaller_model_config: dict | None = None):
    """
    ...
    if smaller_model_config:
        ...
        architecture_config = {
            ...
            "arch_kwargs": {
                ...
                "n_conv_per_stage_decoder": smaller_model_config["n_conv_per_stage_decoder"],
                "conv_bias": True,
    ...
    if smaller_model_config:
        ...
        print(f"   Parameters: {smaller_model_config['n_stages']} stages, {sum(smaller_model_config['features_per_stage'])} total features")
    else:
...
def fix_resenc_no_resampling(dataset_id: int, patch_size: Tuple[int, int], batch_size: int, config_name: str):
    """
...
def main():
    ...
    if SMALLER_MODEL_CONFIG:
        print(f"Model Size: SMALLER ({SMALLER_MODEL_CONFIG['n_stages']} stages, {sum(SMALLER_MODEL_CONFIG['features_per_stage'])} features)")
    else:
    ...
    if not create_fresh_resenc_plans(DATASET_ID, TILING_SIZE, BATCH_SIZE, MODEL_NAME, SMALLER_MODEL_CONFIG):
    ...
    try:
        splits_file = create_stratified_split(DATASET_ID, RANDOM_SEED, STRATIFIED_SPLIT_PERCENTAGE)
    ...
    if SMALLER_MODEL_CONFIG:
        print(f"✅ SMALLER model architecture ({SMALLER_MODEL_CONFIG['n_stages']} stages, {sum(SMALLER_MODEL_CONFIG['features_per_stage'])} features)")
