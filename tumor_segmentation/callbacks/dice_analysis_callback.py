...
class DiceAnalysisCallback(Callback):
        ...
        if trainer.current_epoch % self.analysis_every_n_epochs == 0:
            ...
            for i in range(batch_size):
                try:
                    ...
                    if hasattr(dataset, 'all_images') and global_idx < len(dataset.all_images):
                        img_path = dataset.all_images[global_idx]
                        sample_name = Path(img_path).stem  # Get filename without extension
                    else:
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        ...
        if trainer.current_epoch % self.analysis_every_n_epochs == 0 and self.validation_predictions:
            ...
            for i in range(len(predictions)):
                ...
                # Calculate FP and FN
                fp = np.sum(pred_binary * (1 - target_binary))  # Predicted but not in target
                fn = np.sum((1 - pred_binary) * target_binary)  # In target but not predicted
                fp_counts.append(fp)
                fn_counts.append(fn)
            ...
            worst_patient_indices = np.argsort(dice_scores)[:min(self.save_top_k, len(dice_scores))]
            worst_indices = [patient_indices[idx] for idx in worst_patient_indices]
            
            # Best performing (highest dice scores)
            best_patient_indices = np.argsort(dice_scores)[::-1][:min(self.save_top_k, len(dice_scores))]
            ...
            median_patient_indices = np.argsort(median_distances)[:min(self.save_top_k, len(dice_scores))]
            ...
            self._save_comprehensive_visualization(
                images, targets, predictions, names,
                worst_indices, best_indices, median_indices,
                worst_patient_indices, best_patient_indices, median_patient_indices,
                dice_scores,
    ...
    def _save_dice_csv(self, patient_names: List[str], dice_scores: np.ndarray, epoch: int):
        ...
        # Create DataFrame
        all_epochs = sorted(set(epoch for patient_data in self.patient_dice_history.values() for epoch in patient_data.keys()))
        ...
        for patient_name in sorted(self.patient_dice_history.keys()):
            ...
            for e in all_epochs:
                if e in self.patient_dice_history[patient_name]:
                    data[f'Epoch_{e:03d}'].append(f"{self.patient_dice_history[patient_name][e]:.6f}")
                else:
                    data[f'Epoch_{e:03d}'].append('')  # Empty cell if patient not present in that epoch
